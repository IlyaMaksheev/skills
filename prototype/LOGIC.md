...
Put the actual logic (the bit that's answering the question) in a single `<script>` block written as a small, pure module that could inform the real codebase later. The page around it is throwaway; the module preserves the validated model for a separate implementation task.
...
Open the file and exercise every guided walkthrough, including its reset and state updates, plus the free-play controls. This is runtime verification, not a maintained test suite. Record actual checks or unavailable checks in `RESULT.md` as described in [SKILL.md](SKILL.md).
...
After the human settles the question, capture the answer in `RESULT.md` and preserve the prototype the way [SKILL.md](SKILL.md) describes. The validated reducer / machine / function set is evidence for later implementation, not a production change to make now. The HTML shell stays with it under `.agent-docs` on the mixed feature branch, and being one self-contained file, it stays trivially re-runnable there.

## Anti-patterns

- **Don't add tests.** A prototype that needs tests is no longer a prototype.
- **Don't wire it to the real database.** Use in-memory state; persistence questions follow the scratch-storage rule in [SKILL.md](SKILL.md).
